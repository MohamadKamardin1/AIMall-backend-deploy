...
_CLOUD_TASKS_API_RESOURCE_PATH = \
    'projects/{project_id}/location/{location_id}/queues/{resource_id}/tasks'
...
def task_queue(
    ...
    The function name can be either:
        1. A fully qualified function resource name:
            `projects/{project-id}/location/{location-id}/functions/{function-name}`

        2. A partial resource name with location and function name, in which case
            the runtime project ID is used:
            `location/{location-id}/functions/{function-name}`
...
class TaskQueue:
    ...
    def _parse_resource_name(self, resource_name: str, resource_id_key: str) -> Resource:
        ...
        reg = f'^(projects/([^/]+)/)?location/([^/]+)/{resource_id_key}/([^/]+)$'
...
class TaskOptions:
    ...
    Args:
        ...
        headers: HTTP request headers to include in the request to the task queue function. These
            ...
            https://cloud.google.com/tasks/docs/reference/rest/v2/projects.location.queues.tasks#httprequest
...
class Task:
    ...
    https://cloud.google.com/tasks/docs/reference/rest/v2/projects.location.queues.tasks#resource:-task
