...
class BaseQuery(object):
    ...
    Args:
        ...
        order (Optional[Tuple[:class:`google.cloud.firestore_v1.\
        ...
        start_at (Optional[Tuple[dict, bool]]):
            ...
            * an ``after`` flag
            ...
            will be used in the order given by ``order``.
    ...
    def _copy(
        ...
        *,
        ...
        | object = _not_passed,
        order: Optional[Tuple[query.StructuredQuery.Order]] | object = _not_passed,
        ...
        return self.__class__(
            ...
            order=self._evaluate_param(order, self._order),
    ...
    def order_by(
        ...
        new_order = self._order + (order_pb,)
        return self._copy(order=new_order)
    ...
    def _normalize_order(self) -> list:
        """Helper:  adjust order based on cursors, where clauses."""
        ...
        if _has_snapshot_cursor:
            # added order should use direction of last order
            last_direction = order[-1].direction if order else BaseQuery.ASCENDING
            order_keys = [order.field.field_path for order in order]
            for filter_ in self._field_filters:
                ...
                if isinstance(filter_.op, StructuredQuery.FieldFilter.Operator):
                    ...
                    if filter_.op in _INEQUALITY_OPERATORS and field not in order_keys:
                        order.append(self._make_order(field, last_direction))
            # add __name__ if not already in order
            if "__name__" not in [order.field.field_path for order in order]:
                order.append(self._make_order("__name__", last_direction))

        return order

    def _normalize_cursor(self, cursor, order) -> Tuple[List, bool] | None:
        """Helper: convert cursor to a list of values based on order."""
        if cursor is None:
            return None

        if not order:
            raise ValueError(_NO_order_FOR_CURSOR)
        ...
        if document_fields and len(document_fields) > len(order):
    ...
    def _to_protobuf(self) -> StructuredQuery:
        ...
        start_at = self._normalize_cursor(self._start_at, order)
        end_at = self._normalize_cursor(self._end_at, order)
    ...
    def _comparator(self, doc1, doc2) -> int:
        ...
        if len(_order) == 0:
            ...
            if _order[-1].direction == 1:
                lastDirection = BaseQuery.ASCENDING
            else:
                lastDirection = BaseQuery.DESCENDING

        orderBys = list(_order)
...
class BaseCollectionGroup(BaseQuery):
    ...
    def _prep_get_partitions(
        ...
        query = klass(
            self._parent,
            order=self._PARTITION_QUERY_ORDER,
...
class QueryPartition:
    ...
    def query(self):
        ...
        return type(query)(
            ...
            order=query._PARTITION_QUERY_ORDER,
            start_at=start_at,
            end_at=end_at,
        )
