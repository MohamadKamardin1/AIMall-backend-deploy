...
NO_OP_TRUST_BOUNDARY_location: List[str] = []
NO_OP_TRUST_BOUNDARY_ENCODED_location = "0x0"
...
class CredentialsWithTrustBoundary(Credentials):
    ...
    def with_trust_boundary(self, trust_boundary):
        ...
        Args:
            ...
            credential. This should be a map with a "location" key that maps to
            a list of GCP regions, and a "encodedlocation" key that maps to a
            hex string.
    ...
    def _get_trust_boundary_header(self):
        if self._trust_boundary is not None:
            if self._has_no_op_trust_boundary():
                ...
                return {"x-allowed-location": ""}
            else:
                return {"x-allowed-location": self._trust_boundary["encodedlocation"]}
        return {}
    ...
    def _has_no_op_trust_boundary(self):
        # A no-op trust boundary is indicated by encodedlocation being "0x0".
        # The "location" list may or may not be present as an empty list.
        if self._trust_boundary is None:
            return False
        return (
            self._trust_boundary.get("encodedlocation")
            == NO_OP_TRUST_BOUNDARY_ENCODED_location
        )
